...
class GeneratorPanel():
    ...
    def __init__(
        ...
        self._custom_headers = custom_headers if custom_headers is not None else {}
        self._scraped_headers = scraped_headers if scraped_headers is not None else {}
        ...
        self._omnibar = OmnibarMultipleActions(
            hint=DEFAULT_LOAD_URL,
            labels=["Load","Set Headers"],
            actions=[self._loadurl, self._select_headers],
        )
    ...
    def _select_headers(self, evt):
        ...
        logging.debug("The target for the selected headers is: %s" % target)
        ...
        logging.debug("The host for the url %s is %s" % (target, host))

        # check if there are scraped header for that host
        if host not in self._scraped_headers.keys():
