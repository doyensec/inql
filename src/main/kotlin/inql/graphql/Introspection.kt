...
fun sendIntrospectionQuery(url: String, headers: List<HttpHeader>): String? {
    val urlPath = URI.create(url).path
    val request = HttpRequest.httpRequest()
        .withService(HttpService.httpService(url))
        .withMethod("POST")
        .withPath(urlPath)
        .withUpsertedHeaders(headers.associate { it.name() to it.value() })
    return sendIntrospectionQuery(request)
}

private fun sendIntrospectionQuery(request: HttpRequest): String? {
...
private fun sendIntrospectionQueryVersion(request: HttpRequest, version: IntrospectionQuery.Version): String? {
    val gson = Gson()
